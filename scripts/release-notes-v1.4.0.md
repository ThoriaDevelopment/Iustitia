...
`throughWalls` never actually worked. Its line-of-sight helper handed the ray context a null entity, which makes `World.raycast` throw, and the throw was swallowed by the usual fail-open catch and read as "clear" — so the check was structurally incapable of observing an occluded hit, on any wall, ever. The ray now gets the observer's own entity, which selects entity-dependent collision shapes and never bends the ray itself. Occlusion also reads the victim's whole body (eye, torso, feet) rather than a single eye-to-eye ray, and flags on a rolling match rate instead of the first blocked hit, so one fragile verdict neither flags a legit player nor deflates the rate.

`noFallDamage` was rebuilt around a launch floor, which fixes two false positives and closes a spoof gap in one move. A fall caught on water, slime, hay or cobweb reduces damage the vanilla way and doesn't accumulate VL, and a flight launched by a wind charge or a TNT cannon re-bases the accumulator at the impulse itself, so only the descent below the launch point counts, which is exactly the part vanilla still damages. A landing after a mace smash re-bases it the same way. None of that comes free: an impulse fired on the way down sits above the player, so it can only raise the floor and never lower it, and it never clears the accumulator either, so a faked burst on a 28-block descent still flags at touchdown.
