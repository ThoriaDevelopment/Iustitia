...
Last full run: **92 scenarios** at `dce93fd` (2026-09-16), 22 legit incl. the smoke test, 60 cheat, 10 replay incl. the three preset gates. All 92 green in a single unsharded pass, **0 false positives, 0 bypasses**, with the same 4 detector gaps and 5 harness-gap entries as the pass before it. The raw report is `build/selftest-report.json`.
